...
The `home-routes` handler in the `galeri.routes.home` namespace
...
The `home-page` function will in turn call the `galeri.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/galeri/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `galeri.handler` namespace:
...
Request middleware functions are located under the `galeri.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `galeri.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `galeri.db.core` in some other namespace.
